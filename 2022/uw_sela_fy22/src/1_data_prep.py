...
from ct_snippets.load_sf_class import SF_SOQL
import numpy as np
...
def zip_code_search(zip_code):
    try:
        county = search.by_zipcode(zip_code).county
        return county
    except:
        return None


df["Parish"] = df.apply(lambda x: zip_code_search(x["C_MailingPostalCode"]), axis=1)
